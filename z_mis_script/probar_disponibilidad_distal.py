    ...
    try:
        ...
        if roomstay is not None:
            ...
            try:
                ...
                print(f"({cont}) ✅ DISPONIBLE: {nombre:<45} ({codigo:<15}) | Destino: {destino:<20} | Ciudad: {ciudad:<6} | Precio: {precio_float:>8.2f} {moneda}")
                resumen_por_destino[destino].append((nombre, codigo, precio_float, moneda))
            except (ValueError, TypeError):
                print(f"⚠️ ERROR: {nombre:<45} ({codigo:<15}) → Precio inválido: {precio_raw}")

        else:
            print(f"({cont}) ❌ NO DISPONIBLE: {hotel.hotel_name:<45} ({hotel.hotel_code:<15})")
            
        cont += 1 

    except Exception as e:
        print(f"⚠️ ERROR: {hotel.hotel_name:<45} ({hotel.hotel_code:<15}) → {str(e)}")
